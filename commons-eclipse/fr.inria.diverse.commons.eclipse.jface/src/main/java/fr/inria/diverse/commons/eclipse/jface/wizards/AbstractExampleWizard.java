 ...
 *    Cyril Faucher
 */
package fr.inria.diverse.commons.eclipse.jface.wizards;
...
/**
 * <p>
 * This abstract example wizard simply unzips a number of zips into the
 * workspace as projects. It does not offer any pages but can be added as a new
 * wizard to the new wizards dialog through the org.eclipse.ui.newWizards
 * extension point.
 * </p>
 * <p>
 * Clients should subclass this class and override the
 * <code>getProjectDescriptor()</code> method to provide the location of the
 * project zips that should be unzipped into the workspace. Note that any
 ...
 * extension point that the wizard declaration should have canFinishEarly = true
 * and hasPages = false. Any label and icon can be freely given to the wizard to
...
public abstract class AbstractExampleWizard extends Wizard implements
	...
	/**
	 * A descriptor class that describes where to find the zipped contents of a
	 * project and what that project should be named when unzipped into the
	 * workspace.
	 */
	public static class ProjectDescriptor {
		...
		/**
		 * Construct a descriptor that points to a zip file located in a
		 * particular bundle at the given location within that bundle. Also
		 * provided is the project name for which the zip is the contents. Note
		 * that this project name should be the same as is in the contents not
		 * some alternative name.
		 * 
		 * @param bundleName
		 *            The bundle in the runtime that contains the zipped up
		 *            project contents.
		 * @param zipLocation
		 *            The location within the bundle where the zip file is
		 *            located.
		 * @param projectName
		 *            The project name in the workspace that will be created to
		 *            house the project contents.
		 */
	...
	public boolean performFinish() {
		...
		WorkspaceJob job = new WorkspaceJob("Unzipping Projects") {
			...
			public IStatus runInWorkspace(IProgressMonitor monitor)
				...
				monitor.beginTask("Unzipping Projects",
	...
	/**
	 * The subclass provides the specific project descriptors for the projects
	 * that should be unzipped into the workspace. Note that any projects that
	 * already exist in the workspace will not be overwritten as they may
	 * contain changed made by the user.
	 * 
	 * @return The collection of project descriptors that should be unzipped
	 *         into the workspace.
	 */
	...
	private void unzipProject(ProjectDescriptor descriptor,
		...
		if (interpreterZipUrl == null) {
			String message = "cannot find " + zipLocation + " in plugin "
					+ bundleName + ". Project " + projectName
					+ " will not be created";
		...
		if (project.exists()) {
			try {
				ActionIfProjectExist action = getActionIfProjectExist(project);
				switch (action) {
				case ASKUSER:
						...
						public void run() {
							try {
								MessageDialog dialog = new MessageDialog(
										...
										"Project "
												+ project.getName()
												+ " already exist. Do you want to:",
										MessageDialog.QUESTION, new String[] {
												"Keep local version",
												"Replace with new version" }, 0); 
								// Keep local version is the default
		...
		try {
			...
			getContainerPlugin().getLog().log(
					new Status(IStatus.ERROR, getContainerPlugin().getBundle()
							.getSymbolicName(), IStatus.ERROR, e.getMessage(),
							e));
		} catch (CoreException e) {
			getContainerPlugin().getLog().log(e.getStatus());
		}
	...
	/**
	 * return the instance of the plugin that contain this wizard (used for log
	 * support)
	 * 
	 * @return
	 */
	...
	public enum ActionIfProjectExist {
		DONOTHING, OVERRIDE, ASKUSER
	}

	public abstract ActionIfProjectExist getActionIfProjectExist(
			IProject project);
