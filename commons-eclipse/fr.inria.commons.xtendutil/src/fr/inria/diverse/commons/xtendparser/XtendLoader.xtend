...
/**
 * Set of hacks relying on XtendBatchCompiler to parse Xtend code and to obtain an Xtend model.
 * Some of the code comes from (modified) org.eclipse.xtend.ide.macro.JdtBasedProcessorProvider
 * 
...
class XtendLoader {
	...
	public def void loadXtendModel(IJavaProject javaProject) {
		...
		for (IFolder f : findSrcFoldersOf(javaProject)) {
	...
	private def static getWorkspaceRoot(IJavaProject javaProject) {
